...
docker exec -it "${_slug}_php" wp plugin install query-monitor
docker exec -it "${_slug}_php" wp plugin install wp-sweep
docker exec -it "${_slug}_php" wp plugin install user-role-editor
...
docker exec -it "${_slug}_php" wp site empty --yes
...
git clone git@github.com:filipecsweb/wp-theme-ss-lite.git ${_wp_content_dir}/themes/ss-lite
docker exec -it "${_slug}_php" wp theme activate ss-lite
